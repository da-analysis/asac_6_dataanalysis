...
# API 정보
url = "https://api.odcloud.kr/api/15089540/v1/uddi:2c4de1aa-e2a2-4708-8c01-b7c54818c9fc"
service_key = dbutils.secrets.get(scope="asac_6", key="datago")
catalog_table = "bronze.api_public.`보령사랑상품권_판매대행점현황_보령_20240829`"
...
# 데이터 스키마 정의
schema = StructType([
    StructField("연번", StringType(), True),
    StructField("지점명", StringType(), True),
    StructField("소재지_도로명주소", StringType(), True),
    StructField("소재지_지번주소", StringType(), True),
    StructField("위도", StringType(), True),
    StructField("경도", StringType(), True),
    StructField("전화번호", StringType(), True),
    StructField("데이터기준일", StringType(), True),
...
# MAGIC %sql
# MAGIC SELECT *
# MAGIC FROM bronze.api_public.`보령사랑상품권_판매대행점현황_보령_20240829`
...
# MAGIC SELECT COUNT(*)
# MAGIC FROM(
# MAGIC SELECT DISTINCT(*) FROM bronze.api_public.`보령사랑상품권_판매대행점현황_보령_20240829`
# MAGIC ORDER BY id);

...
# API 정보
url = "https://api.odcloud.kr/api/15096095/v1/uddi:9c98789d-0b8b-4895-8d44-edb175823355"
service_key = dbutils.secrets.get(scope="asac_6", key="datago")
catalog_table = "bronze.api_public.`소상공인_지역화폐_취급현황_경상북도_20211214`"
...
# 데이터 스키마 정의
schema = StructType([
    StructField("순번", StringType(), True),
    StructField("지역화폐_분류", StringType(), True),
    StructField("상호명", StringType(), True),
    StructField("시군", StringType(), True),
    StructField("도로명_주소", StringType(), True),
...
# MAGIC %sql
# MAGIC SELECT *
# MAGIC FROM bronze.api_public.`소상공인_지역화폐_취급현황_경상북도_20211214`
...
# MAGIC SELECT COUNT(*)
# MAGIC FROM(
# MAGIC SELECT DISTINCT(*) FROM bronze.api_public.`소상공인_지역화폐_취급현황_경상북도_20211214`
# MAGIC ORDER BY id);

...
# API 정보
url = "https://api.odcloud.kr/api/15111366/v1/uddi:16d6c8c1-37f9-424b-89f4-d6bd59fafcc2"
service_key = dbutils.secrets.get(scope="asac_6", key="datago")
catalog_table = "bronze.api_public.`지역화폐_사용_및_상위_5순위_업종_현황_대전_20211231`"
...
# 데이터 스키마 정의
schema = StructType([
    StructField("연월", StringType(), True),
    StructField("사용금액_원", StringType(), True),
    StructField("사용금액_상위_업종_1순위", StringType(), True),
    StructField("사용금액_상위_업종_2순위", StringType(), True),
    StructField("사용금액_상위_업종_3순위", StringType(), True),
    StructField("사용금액_상위_업종_4순위", StringType(), True),
    StructField("사용금액_상위_업종_5순위", StringType(), True),
...
# MAGIC %sql
# MAGIC SELECT *
# MAGIC FROM bronze.api_public.`지역화폐_사용_및_상위_5순위_업종_현황_대전_20211231`
...
# MAGIC SELECT COUNT(*)
# MAGIC FROM(
# MAGIC SELECT DISTINCT(*) FROM bronze.api_public.`지역화폐_사용_및_상위_5순위_업종_현황_대전_20211231`
# MAGIC ORDER BY id);

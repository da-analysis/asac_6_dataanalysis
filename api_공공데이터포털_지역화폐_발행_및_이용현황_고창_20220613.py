...
# API 정보
url = "https://api.odcloud.kr/api/15101177/v1/uddi:a07a6fb5-c1e2-4bf7-824d-e4a2cecd320f"
service_key = dbutils.secrets.get(scope="asac_6", key="datago")
catalog_table = "bronze.api_public.`지역화폐_발행_및_이용현황_고창_20220613`"
...
schema = StructType([
    ...
    StructField("발행연월", StringType(), True),
    StructField("발행현황", StringType(), True), # 발행현황(원)
    StructField("이용건수", StringType(), True), # 이용건수(결제건수)
...
# MAGIC %sql
# MAGIC SELECT *
# MAGIC FROM bronze.api_public.`지역화폐_발행_및_이용현황_고창_20220613`
...
# MAGIC SELECT COUNT(*)
# MAGIC FROM(
# MAGIC SELECT DISTINCT(*) FROM bronze.api_public.`지역화폐_발행_및_이용현황_고창_20220613`
# MAGIC ORDER BY id);

...
# API 정보
url = "https://api.odcloud.kr/api/15101756/v1/uddi:89bbf57b-aef2-422e-a477-c72f217463b8"
service_key = dbutils.secrets.get(scope="asac_6", key="datago")
catalog_table = "bronze.api_public.`지역화폐_발행_및_이용현황_진안_20220629`"
...
# 데이터 스키마 정의
schema = StructType([
    StructField("시도명", StringType(), True),
    StructField("시군구", StringType(), True),
    StructField("지역화폐명", StringType(), True),
    StructField("발행연도", StringType(), True),
    StructField("발행액", StringType(), True),
    StructField("판매액", StringType(), True),
    StructField("업종", StringType(), True),
    StructField("건수", StringType(), True),
    StructField("금액", StringType(), True),
    StructField("참고사항", StringType(), True),
...
# MAGIC %sql
# MAGIC SELECT *
# MAGIC FROM bronze.api_public.`지역화폐_발행_및_이용현황_진안_20220629`
...
# MAGIC SELECT COUNT(*)
# MAGIC FROM(
# MAGIC SELECT DISTINCT(*) FROM bronze.api_public.`지역화폐_발행_및_이용현황_진안_20220629`
# MAGIC ORDER BY id);

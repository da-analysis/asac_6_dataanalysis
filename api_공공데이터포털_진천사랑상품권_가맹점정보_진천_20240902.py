...
# API 정보
url = "https://api.odcloud.kr/api/15118382/v1/uddi:3f138db2-bd06-4e11-b29e-8622613a06ad"
service_key = dbutils.secrets.get(scope="asac_6", key="datago")
catalog_table = "bronze.api_public.`진천사랑상품권_가맹점정보_진천_20240902`"
...
schema = StructType([
    ...
    StructField("가맹점기본주소_도로명", StringType(), True),
    StructField("유형", StringType(), True),
    StructField("기준일", StringType(), True),
...
# MAGIC %sql
# MAGIC SELECT *
# MAGIC FROM bronze.api_public.`진천사랑상품권_가맹점정보_진천_20240902`
...
# MAGIC SELECT COUNT(*)
# MAGIC FROM(
# MAGIC SELECT DISTINCT(*) FROM bronze.api_public.`진천사랑상품권_가맹점정보_진천_20240902`
# MAGIC ORDER BY id);

...
spark = SparkSession.builder \
    .appName("API Data Collection for TBDASSIGNRMSALESPUBCATE") \
    .getOrCreate()

# API 정보
url = "https://openapi.gg.go.kr/TBDASSIGNRMSALESPUBCATE"
catalog_table = "bronze.api_gyeonggi.`분석시스템_경기도_지역화폐_발행구분별_집계_경기`"

# 데이터 스키마 정의
schema = StructType([
    StructField("STD_YM", StringType(), True),
    StructField("SIGN_CD", StringType(), True),
    StructField("PUB_CATEGORY_CD", StringType(), True),
    ...
    while retries < 3:
        try:
            ...
            try:
                ...
                code = result['TBDASSIGNRMSALESPUBCATE'][0]['head'][1]['RESULT']['CODE']
                if code == 'INFO-000':  # 정상 응답
                    items = result['TBDASSIGNRMSALESPUBCATE'][1]['row']
                    break
                else:
...
# MAGIC %sql
# MAGIC ALTER TABLE bronze.api_gyeonggi.`분석시스템_경기도_지역화폐_발행구분별_집계_경기`
# MAGIC ALTER COLUMN STD_YM COMMENT '기준년월';
# MAGIC
# MAGIC ALTER TABLE bronze.api_gyeonggi.`분석시스템_경기도_지역화폐_발행구분별_집계_경기`
# MAGIC ALTER COLUMN SIGN_CD COMMENT '시군코드';
# MAGIC
# MAGIC ALTER TABLE bronze.api_gyeonggi.`분석시스템_경기도_지역화폐_발행구분별_집계_경기`
# MAGIC ALTER COLUMN PUB_CATEGORY_CD COMMENT '발행구분코드(전체:9)';
# MAGIC
# MAGIC ALTER TABLE bronze.api_gyeonggi.`분석시스템_경기도_지역화폐_발행구분별_집계_경기`

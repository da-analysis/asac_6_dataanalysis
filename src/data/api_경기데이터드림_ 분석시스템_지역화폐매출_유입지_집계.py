...
# API 정보
url = "https://openapi.gg.go.kr/TBDASRMSALESINFLS"
catalog_table = "bronze.api_gyeonggi.`분석시스템_지역화폐매출_유입지_집계_경기`"
...
schema = StructType([
    ...
    StructField("INFL_ADMDONG_CD", StringType(), True),
    StructField("SALES_AMT", StringType(), True),
    StructField("SALES_AMT_RKI", StringType(), True),
    ...
    StructField("OPER_YN", StringType(), True),
    ...
    while retries < 3:
        try:
            ...
            try:
                ...
                code = result['TBDASRMSALESINFLS'][0]['head'][1]['RESULT']['CODE']
                if code == 'INFO-000':  # 정상 응답
                    items = result['TBDASRMSALESINFLS'][1]['row']
                    break
                else:
...
# MAGIC %sql
# MAGIC ALTER TABLE bronze.api_gyeonggi.`분석시스템_지역화폐매출_유입지_집계_경기`
...
# MAGIC
# MAGIC ALTER TABLE bronze.api_gyeonggi.`분석시스템_지역화폐매출_유입지_집계_경기`
# MAGIC ALTER COLUMN INFL_ADMDONG_CD COMMENT '유입지행정동코드';
# MAGIC
# MAGIC ALTER TABLE bronze.api_gyeonggi.`분석시스템_지역화폐매출_유입지_집계_경기`
# MAGIC ALTER COLUMN SALES_AMT COMMENT '매출금액';
# MAGIC
# MAGIC ALTER TABLE bronze.api_gyeonggi.`분석시스템_지역화폐매출_유입지_집계_경기`
# MAGIC ALTER COLUMN SALES_AMT_RKI COMMENT '매출금액_순위';
# MAGIC
# MAGIC ALTER TABLE bronze.api_gyeonggi.`분석시스템_지역화폐매출_유입지_집계_경기`
...
# MAGIC
# MAGIC ALTER TABLE bronze.api_gyeonggi.`분석시스템_지역화폐매출_유입지_집계_경기`
# MAGIC ALTER COLUMN OPER_YN COMMENT '개방여부';
# MAGIC

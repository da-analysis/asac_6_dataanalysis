...
# API 정보
url = "https://openapi.gg.go.kr/TBDASRMSALESSIGNGUSEXAGE"
catalog_table = "bronze.api_gyeonggi.`분석시스템_지역화폐매출_시군구_성연령별_집계_경기`"

# 데이터 스키마 정의
schema = StructType([
    StructField("STD_YM", StringType(), True),
    StructField("SIGNGU_CD", StringType(), True),
    ...
    while retries < 3:
        try:
            ...
            try:
                ...
                code = result['TBDASRMSALESSIGNGUSEXAGE'][0]['head'][1]['RESULT']['CODE']
                if code == 'INFO-000':  # 정상 응답
                    items = result['TBDASRMSALESSIGNGUSEXAGE'][1]['row']
                    break
                else:
...
# MAGIC %sql
# MAGIC ALTER TABLE bronze.api_gyeonggi.`분석시스템_지역화폐매출_시군구_성연령별_집계_경기`
# MAGIC ALTER COLUMN STD_YM COMMENT '기준년월';
# MAGIC
# MAGIC ALTER TABLE bronze.api_gyeonggi.`분석시스템_지역화폐매출_시군구_성연령별_집계_경기`
# MAGIC ALTER COLUMN SIGNGU_CD COMMENT '시군구코드';
# MAGIC
# MAGIC ALTER TABLE bronze.api_gyeonggi.`분석시스템_지역화폐매출_시군구_성연령별_집계_경기`

...
spark = SparkSession.builder \
    .appName("API Data Collection for TBDASSIGNRMSALESSEXAGES") \
    .getOrCreate()

# API 정보
url = "https://openapi.gg.go.kr/TBDASSIGNRMSALESSEXAGES"
catalog_table = "bronze.api_gyeonggi.`분석시스템_경기도_지역화폐_성연령별_집계_경기`"
    ...
    while retries < 3:
        try:
            ...
            try:
                ...
                code = result['TBDASSIGNRMSALESSEXAGES'][0]['head'][1]['RESULT']['CODE']
                if code == 'INFO-000':  # 정상 응답
                    items = result['TBDASSIGNRMSALESSEXAGES'][1]['row']
                    break
                else:
...
# MAGIC %sql
# MAGIC ALTER TABLE bronze.api_gyeonggi.`분석시스템_경기도_지역화폐_성연령별_집계_경기`

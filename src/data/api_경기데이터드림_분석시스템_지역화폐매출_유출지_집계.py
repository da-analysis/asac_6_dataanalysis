...
spark = SparkSession.builder \
    .appName("API Data Collection for TBDASRMSALESOUTFLS") \
    .getOrCreate()

# API 정보
url = "https://openapi.gg.go.kr/TBDASRMSALESOUTFLS"
catalog_table = "bronze.api_gyeonggi.`분석시스템_지역화폐매출_유출지_집계_경기`"
...
schema = StructType([
    ...
    StructField("OUTFL_ADMDONG_CD", StringType(), True),
    ...
    while retries < 3:
        try:
            ...
            try:
                ...
                code = result['TBDASRMSALESOUTFLS'][0]['head'][1]['RESULT']['CODE']
                if code == 'INFO-000':  # 정상 응답
                    items = result['TBDASRMSALESOUTFLS'][1]['row']
                    break
                else:
...
# MAGIC %sql
# MAGIC ALTER TABLE bronze.api_gyeonggi.`분석시스템_지역화폐매출_유출지_집계_경기`
...
# MAGIC
# MAGIC ALTER TABLE bronze.api_gyeonggi.`분석시스템_지역화폐매출_유출지_집계_경기`
# MAGIC ALTER COLUMN OUTFL_ADMDONG_CD COMMENT '유출지행정동코드';
# MAGIC
# MAGIC ALTER TABLE bronze.api_gyeonggi.`분석시스템_지역화폐매출_유출지_집계_경기`

...
# API 정보
url = "https://api.odcloud.kr/api/15075618/v1/uddi:ef5e45c6-898e-442a-bd61-f72281b6a06b"
service_key = dbutils.secrets.get(scope="asac_6", key="datago")
catalog_table = "bronze.api_public.`지역화폐_결제내역_수원_20201211`"
...
# 데이터 스키마 정의
schema = StructType([
    StructField("기준년월", StringType(), True),
    StructField("시군구명", StringType(), True),
    StructField("읍면동명", StringType(), True),
    StructField("성별", StringType(), True),
    StructField("연령대", StringType(), True),
    StructField("업종명", StringType(), True),
...
# MAGIC %sql
# MAGIC SELECT *
# MAGIC FROM bronze.api_public.`지역화폐_결제내역_수원_20201211`
...
# MAGIC SELECT COUNT(*)
# MAGIC FROM(
# MAGIC SELECT DISTINCT(*) FROM bronze.api_public.`지역화폐_결제내역_수원_20201211`
# MAGIC ORDER BY id);

...
# API 정보
url = "https://api.odcloud.kr/api/15098190/v1/uddi:51748ec3-1b7e-4bbd-8eaa-f3401bc7c7e4"
service_key = dbutils.secrets.get(scope="asac_6", key="datago")
catalog_table = "bronze.api_public.`지역화폐_발급월별_소비금액지수_대전유성_20221231`"
...
# 데이터 스키마 정의
schema = StructType([
    StructField("발급년", StringType(), True),
    StructField("발급월", StringType(), True),
    StructField("광역시도코드", StringType(), True),
    StructField("광역시도명", StringType(), True),
    StructField("시군구코드", StringType(), True),
    StructField("시군구명", StringType(), True),
    StructField("법정동코드", StringType(), True),
    StructField("법정동명", StringType(), True),
    StructField("연령대", StringType(), True),
    StructField("성별", StringType(), True),
    StructField("지수_이용건수", StringType(), True),
    StructField("지수_이용금액", StringType(), True),
...
# MAGIC %sql
# MAGIC SELECT *
# MAGIC FROM bronze.api_public.`지역화폐_발급월별_소비금액지수_대전유성_20221231`
...
# MAGIC SELECT COUNT(*)
# MAGIC FROM(
# MAGIC SELECT DISTINCT(*) FROM bronze.api_public.`지역화폐_발급월별_소비금액지수_대전유성_20221231`
# MAGIC ORDER BY id);

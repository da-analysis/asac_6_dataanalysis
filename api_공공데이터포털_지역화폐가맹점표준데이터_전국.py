...
url = "http://api.data.go.kr/openapi/tn_pubr_public_local_bill_api"
...
url = "http://api.data.go.kr/openapi/tn_pubr_public_local_bill_api"
service_key = dbutils.secrets.get(scope="asac_6", key="datago")
catalog_table = "bronze.api_public.`지역화폐가맹점표준데이터_전국`"
...
# 데이터 스키마 정의 (JSON 필드 이름에 맞게 수정)
schema = StructType([
    StructField("affiliateNm", StringType(), True),
    StructField("localBill", StringType(), True),
    StructField("ctpvNm", StringType(), True),
    StructField("sggNm", StringType(), True),
    StructField("lctnRoadNmAddr", StringType(), True),
    StructField("lctnLotnoAddr", StringType(), True),
    StructField("sectorNm", StringType(), True),
    StructField("mainPrd", StringType(), True),
    StructField("telno", StringType(), True),
    StructField("crtrYmd", StringType(), True),
    StructField("insttCode", StringType(), True),
...
# MAGIC %sql
# MAGIC SELECT *
# MAGIC FROM bronze.api_public.`지역화폐가맹점표준데이터_전국`
...
# MAGIC SELECT COUNT(*)
# MAGIC FROM(
# MAGIC SELECT DISTINCT(*) FROM bronze.api_public.`지역화폐가맹점표준데이터_전국`
# MAGIC ORDER BY id);
...
# MAGIC %sql
# MAGIC
# MAGIC ALTER TABLE bronze.api_public.`지역화폐가맹점표준데이터_전국`
# MAGIC ALTER COLUMN affiliateNm COMMENT '가맹점명';
# MAGIC
# MAGIC ALTER TABLE bronze.api_public.`지역화폐가맹점표준데이터_전국`
# MAGIC ALTER COLUMN localBill COMMENT '사용가능지역화폐';
# MAGIC
# MAGIC ALTER TABLE bronze.api_public.`지역화폐가맹점표준데이터_전국`
# MAGIC ALTER COLUMN ctpvNm COMMENT '시도명';
# MAGIC
# MAGIC ALTER TABLE bronze.api_public.`지역화폐가맹점표준데이터_전국`
# MAGIC ALTER COLUMN sggNm COMMENT '시군구명';
# MAGIC
# MAGIC ALTER TABLE bronze.api_public.`지역화폐가맹점표준데이터_전국`
# MAGIC ALTER COLUMN lctnRoadNmAddr COMMENT '소재지도로명주소';
# MAGIC
# MAGIC ALTER TABLE bronze.api_public.`지역화폐가맹점표준데이터_전국`
# MAGIC ALTER COLUMN lctnLotnoAddr COMMENT '소재지지번주소';
# MAGIC
# MAGIC ALTER TABLE bronze.api_public.`지역화폐가맹점표준데이터_전국`
# MAGIC ALTER COLUMN sectorNm COMMENT '업종명';
# MAGIC
# MAGIC ALTER TABLE bronze.api_public.`지역화폐가맹점표준데이터_전국`
# MAGIC ALTER COLUMN mainPrd COMMENT '주요상품';
# MAGIC
# MAGIC ALTER TABLE bronze.api_public.`지역화폐가맹점표준데이터_전국`
# MAGIC ALTER COLUMN telno COMMENT '전화번호';
# MAGIC
# MAGIC ALTER TABLE bronze.api_public.`지역화폐가맹점표준데이터_전국`
# MAGIC ALTER COLUMN crtrYmd COMMENT '데이터기준일자';
# MAGIC
# MAGIC ALTER TABLE bronze.api_public.`지역화폐가맹점표준데이터_전국`
# MAGIC ALTER COLUMN insttCode COMMENT '제공기관코드';
# MAGIC

...
# API 정보
url = "https://api.odcloud.kr/api/15114808/v1/uddi:b23b02ea-8e63-45d2-b222-b96ce667c776"
service_key = dbutils.secrets.get(scope="asac_6", key="datago")
catalog_table = "bronze.api_public.`지역화폐_연령별_성별_이용현황_고양_20221231`"
...
# 데이터 스키마 정의
schema = StructType([
    StructField("기준연도", StringType(), True),
    StructField("시군명", StringType(), True),
    ...
    StructField("결제취소건수", StringType(), True),
    StructField("결제취소금액", StringType(), True),
    StructField("1회평균결제금액", StringType(), True),
...
# MAGIC %sql
# MAGIC SELECT *
# MAGIC FROM bronze.api_public.`지역화폐_연령별_성별_이용현황_고양_20221231`
...
# MAGIC SELECT COUNT(*)
# MAGIC FROM(
# MAGIC SELECT DISTINCT(*) FROM bronze.api_public.`지역화폐_연령별_성별_이용현황_고양_20221231`
# MAGIC ORDER BY id);

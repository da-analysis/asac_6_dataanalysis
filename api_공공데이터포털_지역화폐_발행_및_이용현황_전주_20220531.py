...
# API 정보
url = "https://api.odcloud.kr/api/15100894/v1/uddi:428811a4-52dd-4d87-b8d3-f5c477e2379c"
service_key = dbutils.secrets.get(scope="asac_6", key="datago")
catalog_table = "bronze.api_public.`지역화폐_발행_및_이용현황_전주_20220531`"
...
schema = StructType([
    ...
    StructField("월별_카드발행수량", StringType(), True),
    StructField("월별_카드충전액", StringType(), True),
    StructField("월별_카드사용액", StringType(), True),
...
# MAGIC %sql
# MAGIC SELECT *
# MAGIC FROM bronze.api_public.`지역화폐_발행_및_이용현황_전주_20220531`
...
# MAGIC SELECT COUNT(*)
# MAGIC FROM(
# MAGIC SELECT DISTINCT(*) FROM bronze.api_public.`지역화폐_발행_및_이용현황_전주_20220531`
# MAGIC ORDER BY id);

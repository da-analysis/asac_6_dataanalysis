...
# API 정보
url = "https://api.odcloud.kr/api/15101751/v1/uddi:ba40ff01-24d5-4d48-8fe9-dd68f8dbc299"
service_key = dbutils.secrets.get(scope="asac_6", key="datago")
catalog_table = "bronze.api_public.`지역화폐_이용현황_군산_20220531`"
...
# 데이터 스키마 정의
schema = StructType([
    StructField("년도", StringType(), True),
    StructField("업종", StringType(), True),
    StructField("분류", StringType(), True),
...
# MAGIC %sql
# MAGIC SELECT *
# MAGIC FROM bronze.api_public.`지역화폐_이용현황_군산_20220531`
...
# MAGIC SELECT COUNT(*)
# MAGIC FROM(
# MAGIC SELECT DISTINCT(*) FROM bronze.api_public.`지역화폐_이용현황_군산_20220531`
# MAGIC ORDER BY id);
